...
function FeatureCard({ feature, align = 'left' }) {
  return (
    <div
      className={`rounded-xl border border-[rgb(var(--navy-rgb))]/10 bg-white px-4 py-3.5 shadow-sm sm:px-5 sm:py-4 ${
        align === 'right' ? 'text-right' : 'text-left'
      }`}
    >
      <h4 className="font-editorial text-sm font-semibold text-esds-navy sm:text-base">{feature.title}</h4>
      <p className="mt-1 font-functional text-xs leading-relaxed text-esds-ink/70 sm:text-sm">
        {feature.description}
      </p>
    </div>
  )
...
export default function AIInterviewPanel() {
  ...
  return (
    <ParallaxSection
      id="ai-panel"
      className="esds-section"
      strength={56}
    >
      <motion.div
        className="mx-auto max-w-7xl rounded-xl border border-[rgb(var(--accent-rgb))]/20 bg-esds-ivory p-5 shadow-sm sm:p-8 md:p-12"
      ...
      >
        <motion.div className="text-center" variants={staggerItem}>
          <p className="esds-eyebrow text-primary">AI Interview Panel</p>
          <h2 className="mt-3 font-display text-balance text-3xl font-semibold uppercase tracking-display text-esds-navy sm:text-4xl md:text-5xl">
            AI Avatar — Your Autonomous Recruiter
          </h2>
          <p className="mx-auto mt-4 max-w-2xl font-functional text-base text-esds-ink/70 sm:text-lg">
          ...
          <div className="mx-auto max-w-5xl px-1 sm:px-2">
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] lg:grid-rows-3 lg:items-center lg:gap-x-8 lg:gap-y-12 xl:gap-x-12">
              <div className="flex flex-col items-center lg:col-start-2 lg:row-span-3 lg:row-start-1 lg:px-4">
                <div className="rounded-full border-4 border-[rgb(var(--accent-rgb))]/25 bg-white p-3 shadow-sm sm:p-4">
                  <video
                    ref={videoRef}
                    src={heroVideo}
                ...
                <div className="-mt-5 rounded-full bg-[rgb(var(--accent-rgb))] px-4 py-1 font-functional text-xs font-semibold text-white sm:text-sm">
                  Tech Lead
                </div>

                <h3 className="mt-4 font-editorial text-2xl font-semibold text-esds-navy sm:text-3xl">Geeta</h3>
                <p className="mt-2 max-w-xs text-center font-functional text-sm text-esds-ink/70 sm:max-w-sm">
